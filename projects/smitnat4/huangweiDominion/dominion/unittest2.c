...
int main (int argc, char** argv)	{
	...
	printf("----------------------------------\n");
	printf("Begin Testing minionaction():\n");
	...
	minionaction(t1choice1,&G2, 0 ,t1player,t1choice2); // call the refactored function with newly created G2
	...
	minionaction(t2choice1,&G4, 0 ,t2player,t2choice2); // call the refactored function with newly game
	...
	memcpy(&G4, &G3, sizeof(struct gameState)); // copy G1 to G2
	minionaction(t3choice1,&G4, 0 ,t3player, t3choice2); // call the refactored function with newly game
	customAssert(G4.handCount[t3playerOpponent], 4);
	// end test 3 ----------------------------------------------------
	
	printf("Test completed for minionaction!\n");
	return 0;
}