...
int main (int argc, char** argv)	{
	...
	printf("----------------------------------\n");
	printf("Begin Testing ambassadoraction():\n");
	...
	ambassadoraction(t1choice1,t1choice2,&G2, 0 ,t1player); // call the refactored function with newly created G2
	int G1SmithyCount = 0;
	...
	int t2player = 0;
	memcpy(&G2, &G1, sizeof(struct gameState)); // copy G1 to G2
	int returnValue = ambassadoraction(t2choice1,t2choice2,&G2, 0 ,t2player); // call the refactored function with newly created G2
	...
	int t3player = 0;
	memcpy(&G2, &G1, sizeof(struct gameState)); // copy G1 to G2
	int returnValue2 = ambassadoraction(t3choice1,t3choice2,&G2, 0 ,t3player); // call the refactored function with newly created G2
	customAssert(returnValue2, -1);

	// end test 3 ----------------------------------------------------
	
	printf("Test completed for ambassadoraction!\n");
	return 0;
}