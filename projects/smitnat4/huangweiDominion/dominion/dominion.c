...
int* kingdomCards(int k1, int k2, int k3, int k4, int k5, int k6, int k7,
  ...
  int initializeGame(int numPlayers, int kingdomCards[10], int randomSeed,
    ...
    int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
    {
      ...
      switch( card )
      {
        ...
        case tribute:
        return tributeaction(state,handPos, tributeRevealedCards);
        
        
        case ambassador:
        //return ambassadoraction(choice1, choice2,state, handPos,currentPayer); // #bug-1 commenting out misspelling bug (currentPlayer spelled wrong) as it doesn't allow program to run at all
        return ambassadoraction(choice1, choice2,state, handPos,currentPlayer);
    ...
    int mineaction (struct gameState *state, int handPos, int trashPos, int gaincard)
    {
      ...
      //discard trashed card
      // adding in declaration for i #bug-2
      int i;
      for (i = 0; i < state->handCount[currentPlayer]; i++)
      {
        if (state->hand[currentPlayer][i] == j)
    ...
    int minionaction(int gainCoins, struct gameState *state, int currentPlayer, int handPos) { // #bug-3 had to add opening "{" character
    //+1 action
    state->numActions++;
    int i;
    int j;
  ...
  int tributeaction(struct gameState *state, int handPos, int *tributeRevealedCards){ // #bug-4 tributerevealedcards wasn't passed through properly
    int currentPlayer = whoseTurn(state);
    int nextPlayer = currentPlayer;
    //if ((state->discardCount[nextPlayer] + state->deckCount[nextPlayer]) < 1) { #bug-7 causing possible segmentation fault (changed < to <=)
	...
	int i; // had to add declaration of i to get to compile correctly #bug-6
      if (state->deckCount[nextPlayer] == 0) {
  ...
  int ambassadoraction(int reveal, int cardcount,struct gameState *state, int handPos, int currentPlayer){
    ...
    int i; // had to add declaration of i to get to compile correctly #bug-5
    for (i = 0; i < state->handCount[currentPlayer]; i++)
    {
